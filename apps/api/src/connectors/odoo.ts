...
export class OdooConnector {
  ...
  async getCashFlow(years:number[],sections:ProfitLossSection[],asOf?:string,bfrSections:BfrSection[]=[]):Promise<CashFlowReport>{
    ...
    // Do not infer dividends from the equity balance: use only the Belgian PCMN
    // appropriation account 694 (remuneration of capital).
    const distributionAccounts=(await this.call(await this.authenticate(),'account.account','search_read',[[['code','=like','694%']]],{fields:['code','name'],order:'code'}) as Account[]).filter(account=>isProfitLossAccountCode(account.code));
    const distributionGroups=distributionAccounts.length?await Promise.all(years.map(async year=>{const end=asOf&&year===Number(asOf.slice(0,4))?asOf:`${year}-12-31`;return this.call(await this.authenticate(),'account.move.line','read_group',[[['parent_state','=','posted'],['date','>=',`${year}-01-01`],['date','<=',end],['account_id','in',distributionAccounts.map(account=>account.id)]],['debit','credit'],['account_id']],{lazy:false}) as Promise<Group[]>;})):[];
    const distributionById=new Map(distributionAccounts.map(account=>[account.id,account])),distributionRows=new Map<string,ProfitLossAccount>();
    distributionGroups.forEach((groups,index)=>groups.forEach(group=>{const account=group.account_id?distributionById.get(group.account_id[0]):undefined;if(!account||!group.account_id)return;const id=String(account.id),row:ProfitLossAccount=distributionRows.get(id)??{id,code:String(account.code),label:String(account.name??group.account_id[1]??'Distribution aux actionnaires'),values:{}};row.values[String(years[index])]=(row.values[String(years[index])]??0)-((group.debit??0)-(group.credit??0));distributionRows.set(id,row)}));
    ...
    const cashBeforeDebt=record('cfads','Cash-flow disponible pour le service de la dette',(year,index)=>(net[String(year)]??0)+(depreciation[String(year)]??0)-nwc[index]+capex[index],"Résultat après impôt + amortissements - variation du BFR - investissements nets.");
