...
type Group = { account_id?: [number, string]; balance?: number };
...
export class OdooConnector {
  ...
  async getCashFlow(years:number[],sections:ProfitLossSection[],asOf?:string):Promise<CashFlowReport>{
    ...
    const change=(values:Record<string,number>,year:number)=>year===years[0]?0:(values[String(year)]??0)-(values[String(year-1)]??0);
    const nwc=years.map(year=>change(asset('inventory'),year)+change(asset('receivables'),year)+change(asset('prepayments'),year)-change(liability('suppliers'),year)-change(liability('tax-social'),year)-change(liability('accruals'),year));
    const capex=years.map(year=>{const movement=change(asset('fixed-assets'),year);return Math.min(0,-movement+(depreciation[String(year)]??0));});
    ...
    return {years,lines:[record('net-result','Résultat après impôts',year=>net[String(year)]??0),record('depreciation','+ Amortissements',year=>depreciation[String(year)]??0),record('nwc','- / + Variation du besoin en fonds de roulement',(_year,index)=>-nwc[index]),record('capex','- Investissements nets (CAPEX)',(_year,index)=>capex[index]),cashBeforeDebt,record('interest','- Charges financières',year=>line('Financier')[String(year)]??0),record('debt-repayment','- Remboursements nets de dettes financières',(_year,index)=>debtRepayment[index]),record('free-cash-flow','Cash-flow libre après dette',(year,index)=>cashBeforeDebt.values[String(year)]+(line('Financier')[String(year)]??0)+debtRepayment[index],"Cash-flow disponible pour le service de la dette - intérêts - remboursements de capital.")],generatedAt:new Date().toISOString(),source:'odoo'};
  }
