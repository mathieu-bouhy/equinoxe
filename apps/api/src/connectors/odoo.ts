...
type Group = { account_id?: [number, string]; balance?: number; debit?:number };
...
export class OdooConnector {
  ...
  async getCashFlow(years:number[],sections:ProfitLossSection[],asOf?:string,bfrSections:BfrSection[]=[]):Promise<CashFlowReport>{
    ...
    const capexDetail=[...capexRows.values()].filter(account=>Object.values(account.values).some(value=>Math.abs(value)>.004)).sort((a,b)=>a.code.localeCompare(b.code));
