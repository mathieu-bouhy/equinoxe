...
const accountAnalyticAllocation=z.object({id:z.string(),companyId:z.string(),odooAccountId:z.string(),accountCode:z.string().regex(/^\d{1,12}$/),accountLabel:z.string(),profitLossSectionId:z.string().nullable(),profitLossSectionLabel:z.string().nullable(),analyticAllocationCodeId:z.string().nullable(),createdAt:z.string(),updatedAt:z.string()});
...
const employeeAnalyticAllocation=z.object({id:z.string(),companyId:z.string(),sourceDocumentId:z.string(),sourceSheet:z.string(),sourceRow:z.number().int().positive(),firstName:z.string(),lastName:z.string(),fullName:z.string(),entryDate:z.string().nullable(),endDate:z.string().nullable().optional(),function:z.string().nullable(),annualSalaryCost:z.number().finite().nullable(),annualCarCost:z.number().finite().nullable(),analyticAllocationCodeId:z.string().nullable(),createdAt:z.string(),updatedAt:z.string()});
...
export class Store {
  users:Collection<User>;companies:Collection<Company>;access:Collection<CompanyAccess>;dashboards:Collection<DashboardDefinition>;integrations:Collection<IntegrationMetadata>;pnlSections:Collection<ProfitLossSection>;pnlSubsections:Collection<ProfitLossSubsection>;historicalBalances:Collection<HistoricalAccountBalance>;reportSettings:Collection<ReportSettings>;bfrSections:Collection<BfrSection>;analyticAllocationCodes:Collection<AnalyticAllocationCode>;accountAnalyticAllocations:Collection<AccountAnalyticAllocation>;spreadsheetSourceDocuments:Collection<SpreadsheetSourceDocument>;employeeAnalyticAllocations:Collection<EmployeeAnalyticAllocation>;timeEntries:Collection<TimeEntry>;billingSettings:Collection<BillingSettings>;medipostBusinessPlanAssumptions:Collection<MedipostBusinessPlanAssumptions>;
  ...
  constructor(dir:string,databaseUrl?:string){
    ...
    this.accountAnalyticAllocations=collection<AccountAnalyticAllocation>('account-analytic-allocations.json',z.array(accountAnalyticAllocation),()=>[]);
  ...
  async bootstrap(){
    ...
    await this.dashboards.mutate(current=>({values:current.some(row=>row.companyId===gimi.id&&row.slug==='analyse-marge')?current:[...current,{id:crypto.randomUUID(),companyId:gimi.id,slug:'analyse-marge',label:'Analyse de marge',order:6,status:'active' as const}],result:null}));
