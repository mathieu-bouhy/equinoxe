...
export const medipostBusinessPlanAssumptions=z.object({growth:forecastValues,rentDifference:forecastValues,outgoingExecutiveSalary:forecastValues,incomingExecutiveSalary:forecastValues,companyValue:z.number().finite(),cashExtraction:z.number().finite(),loanAmount:z.number().finite(),loanYears:z.number().finite().positive(),loanRate:z.number().finite(),workingCapitalRate:z.number().finite(),capexRate:z.number().finite(),rates:z.record(z.string(),forecastValues),updatedAt:z.string(),updatedByUserId:z.string().nullable()});
