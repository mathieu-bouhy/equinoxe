...
export function BfrConfig(){
  const {companies}=useOutletContext<{companies:Company[]}>(),{companySlug}=useParams(),company=companies.find(item=>item.slug===companySlug);
  if(!company)return <ErrorState message="Société de configuration introuvable."/>;
  return <BfrEditor key={company.id} company={company}/>;
}

function BfrEditor({company}:{company:Company}) {
  const qc=useQueryClient(),[draggedId,setDraggedId]=useState<string|null>(null);
  ...
  const save=useMutation({mutationFn:(sections:BfrSection[])=>api.saveBfrSections(company.id,sections),onSuccess:data=>{setDraft(data);qc.setQueryData(['bfr-sections',company.id],data);qc.invalidateQueries({queryKey:['bfr',company.id]})}});
