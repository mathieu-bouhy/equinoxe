...
export function MonthlyEmployeeCosts({employees,keys,kind}:{employees:EmployeeAnalyticAllocation[];keys:AnalyticAllocationCode[];kind:'salary'|'vehicle'}){
  ...
  return <><Card className="analytic-introduction"><div><p className="eyebrow">Présence mensuelle · 2025–2026</p><h2>Coût {label} par mois</h2><p>Coût annuel divisé par 12. Les mois d’entrée et de départ sont inclus en entier ; la personne est exclue dès le mois suivant son départ. Une date de fin vide ne fixe aucune limite.</p></div><span>24 mois</span></Card>
    <Card className="analytic-table-card cost-table-card"><RatioLegend basis={`du coût ${label} mensuel connu`}/><div className="analytic-table-wrap"><table className="analytic-table cost-allocation-table monthly-cost-table" aria-label={`Coût ${label} par mois`}><thead><tr><th scope="col">Mois</th><th scope="col">Coût {label} mensuel</th>{allocationFields.map(field=><th scope="col" key={field.key}>{field.label}</th>)}<th scope="col">Non réparti</th></tr></thead><tbody>
      {rows.map(row=><tr key={row.month} className={row.month.endsWith('-01')?'cost-summary':''}><th scope="row"><strong>{monthLabel(row.month)}</strong><small className="monthly-presence">{row.activeCount} employés présents</small></th><td className="employee-money">{money.format(row.globalTotal)}</td>{allocationFields.map(field=><td className="ratio-value" key={field.key}>{row.globalTotal?percent.format(row.departmentTotals[field.key]/row.globalTotal):'—'}</td>)}<td className="ratio-value">{row.globalTotal?percent.format(row.unallocated/row.globalTotal):'—'}</td></tr>)}
