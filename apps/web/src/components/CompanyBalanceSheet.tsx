...
export function CompanyBalanceSheet({companyId,renderCashFlow}:{companyId:string;renderCashFlow:(cash:CashFlowReport,balance?:BalanceReport)=>ReactNode}){
  const balance=useQuery({queryKey:['balance',companyId],queryFn:()=>api.balanceSheet(companyId)});
  const cash=useQuery({queryKey:['cash-flow',companyId],queryFn:()=>api.cashFlow(companyId)});
  return <div className="balance-dashboard">
    ...
    {cash.data?renderCashFlow(cash.data,balance.data):cash.isLoading?<LoadingState/>:<ErrorState message="Le pont de trésorerie est indisponible. Le bilan reste consultable."/>}
  </div>;
}
